...
def webhook():
    ...
    # Aquí quitamos .P SIEMPRE del símbolo antes de operar con Bybit
    ...
    if "CONFIRMACION LONG REAL" in evento and size == 0:
        ...
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty: {qty}, SL: {stop_loss}")
        send_discord(msg)
        return jsonify({"status": f"CONFIRMACION LONG REAL abierta qty={qty}, SL={stop_loss}", "order": order}), 200

    if "CONFIRMACION SHORT REAL" in evento and size == 0:
        if not is_within_02_percent(precio_actual, ema13):
            msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="⛔ Precio fuera de rango para CONFIRMACION SHORT REAL (no está dentro del 0.2% de la EMA13).")
            send_discord(msg)
        ...
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty: {qty}, SL: {stop_loss}")
        send_discord(msg)
        return jsonify({"status": f"CONFIRMACION SHORT REAL abierta qty={qty}, SL={stop_loss}", "order": order}), 200
    ...
    if "ACTUALIZA SL DINAMICO" in evento and stop_loss:
        if size != 0:
            ...
            msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"SL dinámico actualizado a {stop_loss} qty={abs(size)}")
            send_discord(msg)
            return jsonify({"status": f"SL dinámico actualizado a {stop_loss}", "sl_result": sl_result}), 200
        else:
            msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="⚠️ No hay posición abierta para actualizar SL dinámico")
            send_discord(msg)
    ...
    if "TP1" in evento and size != 0:
        if symbol in PENDING_LIMITS:
            try:
                bybit.cancel_order(category="linear", symbol=symbol, orderId=PENDING_LIMITS[symbol]["orderId"])
            except Exception as e:
        ...
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP1: {qty_tp1} y limit cancelada")
        send_discord(msg)
    ...
    if "TP2" in evento and size != 0:
        ...
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP2: {qty_tp2}")
        send_discord(msg)

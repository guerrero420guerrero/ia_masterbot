...
def webhook():
    ...
    if "ENTRADA LONG CRUCE VOLUMEN" in evento and size == 0:
        ...
        try:
            ...
            error_msg = f"❌ ERROR al colocar orden LONG CRUCE VOLUMEN: {e}\nSímbolo: {symbol}, Qty: {qty}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty: {qty}, SL: {stop_loss}")
        send_discord(msg)
        return jsonify({"status": f"ENTRADA LONG CRUCE VOLUMEN abierta qty={qty}, SL={stop_loss}", "order": order}), 200
    ...
    if "ENTRADA SHORT CRUCE VOLUMEN" in evento and size == 0:
        ...
        try:
            ...
            error_msg = f"❌ ERROR al colocar orden SHORT CRUCE VOLUMEN: {e}\nSímbolo: {symbol}, Qty: {qty}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty: {qty}, SL: {stop_loss}")
        send_discord(msg)
        return jsonify({"status": f"ENTRADA SHORT CRUCE VOLUMEN abierta qty={qty}, SL={stop_loss}", "order": order}), 200
    ...
    if "ACTUALIZA SL DINAMICO LONG" in evento and size != 0:
        ...
        send_discord(msg)
        return jsonify({"status": f"SL dinámico LONG actualizado a {stop_loss}", "sl_result": sl_result}), 200
    ...
    if "ACTUALIZA SL DINAMICO SHORT" in evento and size != 0:
        ...
        try:
            ...
            error_msg = f"❌ ERROR SL DINÁMICO SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}, SL: {stop_loss}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        ...
        send_discord(msg)
        return jsonify({"status": f"SL dinámico SHORT actualizado a {stop_loss}", "sl_result": sl_result}), 200
    ...
    if "CONFIRMACION SHORT REAL" in evento and size == 0:
        ...
        try:
            ...
            # === TP1 REENTRADA TP3 EMA13 LONG ===
    if "TP1 REENTRADA TP3 EMA13 LONG" in evento and size > 0:
        ...
        try:
            ...
            error_msg = f"❌ ERROR TP1 REENTRADA TP3 EMA13 LONG: {e}\nSímbolo: {symbol}, Qty: {qty_tp1}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty_tp1, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP1 reentrada: {qty_tp1}")
        send_discord(msg)
        return jsonify({"status": "TP1 reentrada ejecutado", "order": order}), 200

    # === TP2 REENTRADA TP3 EMA13 LONG ===
    if "TP2 REENTRADA TP3 EMA13 LONG" in evento and size > 0:
        ...
        try:
            ...
            error_msg = f"❌ ERROR TP2 REENTRADA TP3 EMA13 LONG: {e}\nSímbolo: {symbol}, Qty: {qty_tp2}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty_tp2, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP2 reentrada: {qty_tp2}")
        send_discord(msg)
        return jsonify({"status": "TP2 reentrada ejecutado", "order": order}), 200

    # === TP3 REENTRADA TP3 EMA13 LONG - CIERRE ===
    if "TP3 REENTRADA TP3 EMA13 LONG - CIERRE" in evento and size > 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR TP3 REENTRADA TP3 EMA13 LONG - CIERRE: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="TP3 reentrada LONG - cierre total")
        send_discord(msg)
        return jsonify({"status": "TP3 reentrada LONG - cierre total ejecutado", "order": order}), 200

    # === SL REENTRADA TP3 EMA13 LONG ===
    if "SL REENTRADA TP3 EMA13 LONG" in evento and size > 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR SL REENTRADA TP3 EMA13 LONG: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL reentrada LONG - cierre total")
        send_discord(msg)
        return jsonify({"status": "SL reentrada LONG - cierre total ejecutado", "order": order}), 200

    # === TP1 REENTRADA TP3 EMA13 SHORT ===
    if "TP1 REENTRADA TP3 EMA13 SHORT" in evento and size < 0:
        qty_tp1 = abs(size) * PORCENTAJES["TP1"]
        min_qty, step = get_symbol_info(symbol, category="linear")
        qty_tp1 = max(min_qty, (qty_tp1 // step) * step)
        try:
            ...
            error_msg = f"❌ ERROR TP1 REENTRADA TP3 EMA13 SHORT: {e}\nSímbolo: {symbol}, Qty: {qty_tp1}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty_tp1, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP1 reentrada SHORT: {qty_tp1}")
        send_discord(msg)
        return jsonify({"status": "TP1 reentrada SHORT ejecutado", "order": order}), 200

    # === TP2 REENTRADA TP3 EMA13 SHORT ===
    if "TP2 REENTRADA TP3 EMA13 SHORT" in evento and size < 0:
        qty_tp2 = abs(size) * PORCENTAJES["TP2"]
        min_qty, step = get_symbol_info(symbol, category="linear")
        qty_tp2 = max(min_qty, (qty_tp2 // step) * step)
        try:
            ...
            error_msg = f"❌ ERROR TP2 REENTRADA TP3 EMA13 SHORT: {e}\nSímbolo: {symbol}, Qty: {qty_tp2}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": qty_tp2, "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra=f"Qty TP2 reentrada SHORT: {qty_tp2}")
        send_discord(msg)
        return jsonify({"status": "TP2 reentrada SHORT ejecutado", "order": order}), 200

    # === TP3 REENTRADA TP3 EMA13 SHORT - CIERRE ===
    if "TP3 REENTRADA TP3 EMA13 SHORT - CIERRE" in evento and size < 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR TP3 REENTRADA TP3 EMA13 SHORT - CIERRE: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="TP3 reentrada SHORT - cierre total")
        send_discord(msg)
        return jsonify({"status": "TP3 reentrada SHORT - cierre total ejecutado", "order": order}), 200

    # === SL REENTRADA TP3 EMA13 SHORT ===
    if "SL REENTRADA TP3 EMA13 SHORT" in evento and size < 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR SL REENTRADA TP3 EMA13 SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL reentrada SHORT - cierre total")
        send_discord(msg)
        return jsonify({"status": "SL reentrada SHORT - cierre total ejecutado", "order": order}), 200

    # === SL -0.2% LONG ===
    if "SL -0.2% LONG" in evento and size > 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR SL -0.2% LONG: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL -0.2% LONG ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL -0.2% LONG ejecutado", "order": order}), 200

    # === SL +0.2% SHORT ===
    if "SL +0.2% SHORT" in evento and size < 0:
        try:
            order = bybit.place_order(
                category="linear",
                symbol=symbol,
            ...
            error_msg = f"❌ ERROR SL +0.2% SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL +0.2% SHORT ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL +0.2% SHORT ejecutado", "order": order}), 200
        ...
        # === SL TP1 -0.2% LONG ===
    if "SL TP1 -0.2% LONG" in evento and size != 0:
        positionIdx = 1  # LONG
        # Aquí podrías cerrar la posición, mover SL, etc. Adaptar según tu lógica
        ...
        try:
            ...
            error_msg = f"❌ ERROR SL TP1 -0.2% LONG: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL TP1 -0.2% LONG ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL TP1 -0.2% LONG ejecutado", "order": order}), 200

    # === SL TP1 +0.2% SHORT ===
    if "SL TP1 +0.2% SHORT" in evento and size != 0:
        positionIdx = 2  # SHORT
        try:
            order = bybit.place_order(
                category="linear",
            ...
            error_msg = f"❌ ERROR SL TP1 +0.2% SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL TP1 +0.2% SHORT ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL TP1 +0.2% SHORT ejecutado", "order": order}), 200

    # === SL POST TP1 LONG ===
    if "SL POST TP1 LONG" in evento and size != 0:
        positionIdx = 1
        try:
            order = bybit.place_order(
                category="linear",
            ...
            error_msg = f"❌ ERROR SL POST TP1 LONG: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL POST TP1 LONG ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL POST TP1 LONG ejecutado", "order": order}), 200

    # === SL POST TP1 SHORT ===
    if "SL POST TP1 SHORT" in evento and size != 0:
        positionIdx = 2
        try:
            order = bybit.place_order(
                category="linear",
            ...
            error_msg = f"❌ ERROR SL POST TP1 SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL POST TP1 SHORT ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL POST TP1 SHORT ejecutado", "order": order}), 200

    # === SL TP1 EMA34 LONG ===
    if "SL TP1 EMA34 LONG" in evento and size != 0:
        positionIdx = 1
        try:
            order = bybit.place_order(
                category="linear",
            ...
            error_msg = f"❌ ERROR SL TP1 EMA34 LONG: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL TP1 EMA34 LONG ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL TP1 EMA34 LONG ejecutado", "order": order}), 200

    # === SL TP1 EMA34 SHORT ===
    if "SL TP1 EMA34 SHORT" in evento and size != 0:
        positionIdx = 2
        try:
            order = bybit.place_order(
                category="linear",
            ...
            error_msg = f"❌ ERROR SL TP1 EMA34 SHORT: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
            print(error_msg)
            send_discord(error_msg)
            return jsonify({"status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol}), 400
        msg = format_discord_msg(evento, symbol, precio, porcentaje, stop_loss, tp1, tp2, tp3, ema13, extra="SL TP1 EMA34 SHORT ejecutado")
        send_discord(msg)
        return jsonify({"status": "SL TP1 EMA34 SHORT ejecutado", "order": order}), 200

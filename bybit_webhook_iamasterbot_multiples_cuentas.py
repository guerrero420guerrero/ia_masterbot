...
def webhook():
    ...
    for cuenta_name, bybit in BYBIT_CLIENTS.items():
        try:
            ...
            # === BLOQUE PARA CUBRIR TODOS LOS SL Y REENTRADAS COMO EN MONO-CUENTA ===
# --> Agrega esto después del bloque de cancelación (CERRAR/CANCELAR) y antes del mensaje genérico final

            # --- Cierre automático en eventos SL (Stop Loss) y variantes ---
            sl_keywords = [
                "SL LONG EMA13 CRUCE VOLUMEN",
                "SL SHORT EMA13 CRUCE VOLUMEN",
                "SL INICIAL LONG",
                "SL INICIAL SHORT",
                "SL ENTRADA LONG",
                "SL ENTRADA SHORT",
            ...
            # También cubre por substring "SL" (menos falso positivo)
            if (any(kw in evento for kw in sl_keywords) or
                ("SL" in evento and not ("CANCELAR" in evento or "CERRAR" in evento or "CANCELA" in evento))) and size != 0:
                try:
                    if size > 0:
                        bybit.place_order(
                            category="linear",
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR cierre SL: {e}\nSímbolo: {symbol}, Qty: {size}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": size, "symbol": symbol})
                    continue
                    ...
                    for tp_key in ["tp1", "tp2", "tp3"]:
                        ...
                        if orderId:
                            try:
                                ...
                                print(f"[{cuenta_name}] Error cancelando limit {tp_key}: {e}")
                    pending_limits.pop(symbol)
                msg = f"[{cuenta_name}] 🚨 STOP LOSS ejecutado por evento: {evento}. Cierre total y cancelación de limit"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "SL ejecutado y limit cancelada"})
                continue

            # --- Reentrada TP1/TP2/TP3 LONG ---
            if "TP1 REENTRADA TP3 EMA13 LONG" in evento and size > 0:
                ...
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP1 REENTRADA TP3 EMA13 LONG: {e}\nSímbolo: {symbol}, Qty: {qty_tp1}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": qty_tp1, "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP1 reentrada LONG ejecutado Qty: {qty_tp1}"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP1 reentrada LONG ejecutado", "order": order})
                continue
            ...
            if "TP2 REENTRADA TP3 EMA13 LONG" in evento and size > 0:
                ...
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP2 REENTRADA TP3 EMA13 LONG: {e}\nSímbolo: {symbol}, Qty: {qty_tp2}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": qty_tp2, "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP2 reentrada LONG ejecutado Qty: {qty_tp2}"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP2 reentrada LONG ejecutado", "order": order})
                continue
            ...
            if "TP3 REENTRADA TP3 EMA13 LONG - CIERRE" in evento and size > 0:
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP3 REENTRADA TP3 EMA13 LONG - CIERRE: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP3 reentrada LONG - cierre total ejecutado"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP3 reentrada LONG - cierre total ejecutado", "order": order})
                continue

            # --- Reentrada TP1/TP2/TP3 SHORT ---
            if "TP1 REENTRADA TP3 EMA13 SHORT" in evento and size < 0:
                ...
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP1 REENTRADA TP3 EMA13 SHORT: {e}\nSímbolo: {symbol}, Qty: {qty_tp1}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": qty_tp1, "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP1 reentrada SHORT ejecutado Qty: {qty_tp1}"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP1 reentrada SHORT ejecutado", "order": order})
                continue
            ...
            if "TP2 REENTRADA TP3 EMA13 SHORT" in evento and size < 0:
                ...
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP2 REENTRADA TP3 EMA13 SHORT: {e}\nSímbolo: {symbol}, Qty: {qty_tp2}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": qty_tp2, "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP2 reentrada SHORT ejecutado Qty: {qty_tp2}"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP2 reentrada SHORT ejecutado", "order": order})
                continue
            ...
            if "TP3 REENTRADA TP3 EMA13 SHORT - CIERRE" in evento and size < 0:
                try:
                    ...
                    error_msg = f"[{cuenta_name}] ❌ ERROR TP3 REENTRADA TP3 EMA13 SHORT - CIERRE: {e}\nSímbolo: {symbol}, Qty: {abs(size)}"
                    print(error_msg)
                    send_discord(error_msg)
                    resultados.append({"cuenta": cuenta_name, "status": "error", "msg": str(e), "qty": abs(size), "symbol": symbol})
                    continue
                msg = f"[{cuenta_name}] TP3 reentrada SHORT - cierre total ejecutado"
                send_discord(msg)
                resultados.append({"cuenta": cuenta_name, "status": "TP3 reentrada SHORT - cierre total ejecutado", "order": order})
                continue
